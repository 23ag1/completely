...
- **Unattended** (`cmp run` / `cmp run --max N`): fresh `claude -p` per iteration reading the
...
- **Supervised** (`cmp run --mode supervised`): hand off to `/gsd:execute-phase <phase>` (wave
